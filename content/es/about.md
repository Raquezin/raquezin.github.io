---
title: "Sobre mí"
draft: false
---

> **TL;DR**
>
> Soy un estudiante apasionado por la programación, las matemáticas, la IA y los ordenadores, pero sobre todo me encanta aprender y disfruto construyendo cosas.
>
> En mi tiempo libre, me gusta el piragüismo.
>
> **Mejor con:** Python 3 · SQL · Linux · Estructuras de Datos · Algoritmos
>
> **Familiarizado con:** AI / ML · MongoDB · C · React/TypeScript

---

## En el Agua

Soy **piragüista**. También ejerzo como **árbitro de piragüismo**. La disciplina, la constancia y la deportividad que practico en el agua son los mismos valores que aplico en mi trabajo.

## En el Teclado

Mis intereses técnicos giran en torno a la **programación competitiva**, los **hackathons** y la **construcción de comunidad**.

- **Comunidad y Liderazgo:** Administrador del [Club de Algoritmia de la Universidad de Sevilla](https://clubalgoritmiaus.es/news/2025-06-16-resultados-elecciones-caus-2025) (CAUS) y organizador en la Olimpiada Informática de Andalucía (OIA).
- **Hackathons y Competiciones:**
  - [1º Puesto Categoría A/B y 4º Puesto General @ AdaByron de Andalucía](https://www.linkedin.com/posts/fgiraldezc_english-below-mi-primer-ada-byron-el-ugcPost-7455336280516734976-qNdP) (2026).
  - [2º Puesto @ Olimpiada Informática de Andalucía](https://www.informatica.us.es/index.php/noticias/ganadores-de-la-ix-olimpiada-informatica-de-sevilla-2025) (2025).
  - [3º Premio y Premio de Ciberseguridad por INCIBE @ 30ª Edición Start-up Weekend TechStars](https://www.linkedin.com/posts/fgiraldezc_rittai-swsevilla30-sevilla-activity-7398834330963415040-tpg-) (2025).
  - [2º Premio @ Lovable x Young Founders of London Hackathon](https://www.linkedin.com/posts/fgiraldezc_what-can-a-group-of-young-spanish-builders-activity-7421936535396073472-GDUh) (2026).
  - Otras participaciones: HackEurope, HackUPC, NASA SpaceApps, Sputnik.

## Mis Proyectos

- [**PalistApp**](https://www.palistapp.com): Una plataforma para seguir y ver resultados, carreras y competidores en eventos de piragüismo españoles. Desarrollado con Go, PostgreSQL y React/TypeScript. Procesó ~100k solicitudes en 4 días.
- [**Matrix Library**](https://github.com/raquezin/matrix_library): Una implementación en C de una biblioteca de matrices desde cero, esencial para comprender el álgebra lineal y algoritmos de ML.
- [**Snake Algorithm**](https://github.com/raquezin/snake_algorithms): Una implementación en C del clásico juego Snake con un algoritmo greedy.

## Experiencia

- **CTO @ [HomiMatch](https://homimatch.com)** _(Abr 2025 - May 2026)_
- **Becario @ [Universidad de Sevilla](https://www.us.es)** (Dpto. de Electrónica) _(Oct 2025 - Presente)_
- **Profesor @ [AcademiaConoser](https://www.academiaconoser.com)** (Comunicación y Algoritmos) _(Nov 2025 - Ene 2026)_

---

## Cómo Trabajo

- **Lo que me motiva:** Resolver problemas con IA, superar retos difíciles.
- **Lo que me decepciona:** Frameworks complejos e innecesarios y la arrogancia en el trabajo en equipo.
- **Fortalezas:** Aprendizaje rápido.
- **Puntos de mejora:** Experiencia limitada en infraestructura Cloud.
- **Comportamientos conocidos:** Inbox-zero, obsesión por el formateo, demasiado Linux.
